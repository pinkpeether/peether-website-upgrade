...
export default function SloganAnimation() {
  ...
  useEffect(() => {
    ...
    const resetAnimation = () => {
      allChars.forEach((char) => char.classList.remove("visible", "pop"));
      cult.classList.remove("strike", "underline");
      cursorLine1.classList.add("hidden");
      cursorLine2.classList.add("hidden");
    };

    const queue = (callback, wait) => {
    ...
    const runSequence = () => {
      ...
      delay += 400 * M;
      queue(() => { cursorLine1.classList.remove("hidden"); }, delay);
      delay += 300 * M;

      // Step 2: Type "Trust the "
      trustChars.forEach((char) => {
        queue(() => { char.classList.add("visible"); }, delay);
        delay += charSpeedSlow;
      });
      delay += pauseShort;
      ...
      queue(() => { leftBracket.classList.add("visible", "pop"); }, delay);
      delay += pauseMedium;

      // Step 10: Right bracket pops
      queue(() => { rightBracket.classList.add("visible", "pop"); }, delay);
      delay += pauseLong;

      // Step 11: Strikethrough
      queue(() => { cult.classList.add("strike"); }, delay);
      delay += pauseMedium;

      // Step 12: Underline
      queue(() => { cult.classList.add("underline"); }, delay);
    ...
    const startLoop = () => {
      const sequenceDuration = runSequence();
      restartTimeoutId = setTimeout(startLoop, sequenceDuration + REPEAT_PAUSE);
    };
  ...
  return (
    <>
      <style>{`
        .slogan-anim {
          font-size: clamp(13px, 2.9vw, 34px);
